...
class MultitaskBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # The output is a tuple where the first element contains the hidden states
        outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        
        # The last hidden state is the sequence of hidden states for each token
        last_hidden_state = outputs.last_hidden_state  # Shape: (batch_size, seq_len, hidden_size)
        
        # Typically, we use the [CLS] token's embedding for classification tasks
