...
class SimCSEBERT(torch.nn.Module):
    """BERT model for SimCSE training """
    ...
    def supervised_simcse_loss(self, emb_p, emb_pos, emb_neg, has_negative, temperature=0.05, margin=0.3):
        ...
        valid_neg_mask = has_negative.bool().to(device)
        if torch.any(valid_neg_mask):
            ...
            # Ensure same dtype for assignment 
...
class ContrastiveSNLIDataset(Dataset):
    """SNLI and MNLI sentences for contrastive learning"""
    ...
    def _load_sentences(self, snli_path, mnli_path, sample_size):
        ...
        # Format data as (sentence, dummy_label, dummy_sent_id)
...
class SupervisedNLIDataset(Dataset):
    """Dataset for supervised SimCSE : SNLI and MNL"""
    def __init__(self, snli_path, mnli_path, args, sample_size=None):
        self.pairs = []
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            if scaler:
                ...
                # Only update at accumulation boundaries
                if (step + 1) % accumulation_steps == 0:
                    scaler.unscale_(optimizer)
                    torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
                    scaler.step(optimizer)
